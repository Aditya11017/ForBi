...
import SocW from "./assets/images/SOC-w.svg"
      ...
      <div className="rt-prg-container" ref={scrollRef}>
        {items.map((item) => (
          <div key={item.id} className= {`rt-prg-box ${item.cls}`}>
            ...
            <p>{item.descp}</p>
            <Link to={item.symLink}><button className="view-btn">View Details</button></Link>
          </div>
        ))}
      </div>
      <button className="nav-btn right-btn" onClick={() => scroll('right')}><img className="arrow-icon" src={rtArw} alt="right-arrow" /> </button>
      <Link className="course-link" to="CoreTraining" >Checkout All Core Course</Link>
    </div>
  );
}
